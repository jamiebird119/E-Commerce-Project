...
def add_product(request):
    ...
    if request.metho == 'POST':
        form = ProductForm(request.method, request.FILES)
        if form.is_valid():
            form.save()
...
def edit_product(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(
                request, f'Failed to update product {product.name}. Please ensure form is valid.')
    else:
        form = ProductForm(instance=product)
        messages.info(request, f'Editting product {product.name}')
